...
- **2026-04-15**: **Dual-memory architecture documented and claude-mem pointer-skill installed.** Decided not to vendor [thedotmack/claude-mem](https://github.com/thedotmack/claude-mem) because its hooks depend on `$CLAUDE_PLUGIN_ROOT`, it runs a persistent Node+Bun worker service on localhost:37777, it collides with existing vault hooks, and it's AGPL-3.0. Instead installed it as a pointer skill at `.claude/skills/claude-mem/` with a full SKILL.md documenting what it is, how it complements the existing `brain/` manual memory system, install command (`/plugin marketplace add thedotmack/claude-mem`), and troubleshooting. Updated CLAUDE.md with a new "Dual-memory architecture" section explaining when to use durable `brain/` vs tactical claude-mem auto-capture. Captured the full vendor-vs-install-at-user-level rationale in brain/Key Decisions.md for future reference.
